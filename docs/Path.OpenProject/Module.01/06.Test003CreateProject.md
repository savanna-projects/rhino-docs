...
:arrow_backward: [Previous](./05.Test002UpdateProjectById.md) Unit 6 of 14 [Next](./07.Test004DeleteProject.md) :arrow_forward:
...
2. `date` - a macro for getting the current date and time. The `format` switch allows us to provide a [date-time format](https://docs.microsoft.com/en-us/dotnet/standard/base-types/standard-date-and-time-format-strings).
...
5. `get request` - sends an HTTP `GET` request to `OpenProject` server on address `http://openproject/api/v3/projects/<project_id>` to get that has been updated by the previous `POST` request.
...
2. `response body` - asserts that the name of the project found by the given [JPath (JSON Path)](https://goessner.net/articles/JsonPath/) in the response body of the previous request is equal to the unique name we have created for the project.  

## Next Unit: "Test 004: API - Delete Project"

[Continue](./07.Test004DeleteProject.md) :arrow_forward:
