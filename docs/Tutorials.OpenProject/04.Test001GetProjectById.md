...
1. `GetRequest` - sends an HTTP `GET` request to `OpenProject` server on address `http://openproject/api/v3/projects`.
2. `RegisterResponseParameter` - gets the project id value by using [JPath (JSON Path)](https://goessner.net/articles/JsonPath/) on the response body.
3. `CloseBrowser` - closes the current driver session, without it the driver session will remain available until the next `Rhino API` restart.
...
1. `status_code` - asserts the HTTP StatusCode that was returned by the last HTTP request action.
2. `text` - asserts a static text taken from a parameter, element, or element attribute.
