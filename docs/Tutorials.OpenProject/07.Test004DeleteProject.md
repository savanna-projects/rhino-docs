...
6. `send delete request` - send an HTTP `DELETE` request to `OpenProject` server on address `http://openproject/api/v3/projects/<project_id>` to delete the project that has been created. Please note that the body of the request is empty - this is due to `OpenProject` endpoint expecting body with a `DELETE` request.
